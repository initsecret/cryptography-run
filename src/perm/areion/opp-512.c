...
/* This software is updated based on the algorithms designed in the following
 * research paper. */
...
#define OPP_N (OPP_W * 4) /* nonce size */
...
static OPP_INLINE void opp_pad(uint8_t *out,
                               const uint8_t *in,
                               const size_t inlen) {
...
static OPP_INLINE void opp_init_mask(opp_state_t mask,
                                     const unsigned char *k,
                                     const unsigned char *n) {
  memcpy(&mask->S[0], n, 32);
  memcpy(&mask->S[4], k, 32);
...
static OPP_INLINE void opp_absorb_block(opp_state_t state,
                                        opp_state_t mask,
                                        const uint8_t *in) {
  size_t i;
...
static OPP_INLINE void opp_absorb_lastblock(opp_state_t state,
                                            opp_state_t mask,
                                            const uint8_t *in,
                                            size_t inlen) {
  size_t i;
...
static OPP_INLINE void opp_encrypt_block(opp_state_t state,
                                         opp_state_t mask,
                                         uint8_t *out,
                                         const uint8_t *in) {
  size_t i;
...
static OPP_INLINE void opp_encrypt_lastblock(opp_state_t state,
                                             opp_state_t mask,
                                             uint8_t *out,
                                             const uint8_t *in,
                                             size_t inlen) {
  ...
  /* XOR padded message to state, XOR padded message and mask to block, and
   * extract ciphertext */
  opp_pad(lastblock, in, inlen);
  for (i = 0; i < WORDS(OPP_B); ++i) {
    S[i] ^= LOAD(lastblock + i * BYTES(OPP_W));
    STORE(lastblock + i * BYTES(OPP_W),
          B[i] ^ L[i] ^ LOAD(lastblock + i * BYTES(OPP_W)));
  }
...
static OPP_INLINE void opp_decrypt_block(opp_state_t state,
                                         opp_state_t mask,
                                         uint8_t *out,
                                         const uint8_t *in) {
  size_t i;
...
static OPP_INLINE void opp_decrypt_lastblock(opp_state_t state,
                                             opp_state_t mask,
                                             uint8_t *out,
                                             const uint8_t *in,
                                             size_t inlen) {
...
static void opp_absorb_data(opp_state_t state,
                            opp_state_t mask,
...
static void opp_encrypt_data(opp_state_t state,
                             opp_state_t mask,
                             unsigned char *out,
                             const unsigned char *in,
                             size_t inlen) {
  // FIXME: write an optimized encrypt function
  opp_gamma(mask);
  while (inlen >= BYTES(OPP_B)) {
...
static void opp_decrypt_data(opp_state_t state,
                             opp_state_t mask,
                             unsigned char *out,
                             const unsigned char *in,
                             size_t inlen) {
...
static void opp_finalise(opp_state_t sa,
                         opp_state_t se,
                         opp_state_t mask,
                         unsigned char *tag) {
  size_t i;
...
static int opp_verify_tag(const unsigned char *tag1,
...
void encrypt_areion_512_opp(uint8_t *c,
...
int partial_decrypt_areion_512_opp(
    uint8_t *m,
    uint8_t tag[AREION_512_OPP_TAG_LEN],
    const uint8_t *h,
    size_t hlen,
...
int decrypt_areion_512_opp(uint8_t *m,
...
bool initialize_areion_512_opp(bool enc,
                               const uint8_t *n,
                               const uint8_t *k,
...
bool update_areion_512_opp(uint8_t *out,
                           size_t *olen,
                           size_t olimit,
                           const uint8_t *in,
                           size_t ilen,
...
bool finalize_areion_512_opp(uint8_t *out,
                             size_t *olen,
                             size_t olimit,
                             uint8_t *tag,
                             areion_512_opp_t *state) {
  ...
  *olen = 0;
  if (state->ad_partial_len > 0) {
    opp_beta(state->La);
    opp_absorb_lastblock(state->Sa, state->La, state->ad_buf,
                         state->ad_partial_len);
    state->ad_partial_len = 0;
  }
  ...
  if (state->partial_len > 0) {
    opp_beta(state->Le);
    if (state->enc) {
      opp_encrypt_lastblock(state->Se, state->Le, out, state->buf,
                            state->partial_len);
    } else {
      opp_decrypt_lastblock(state->Se, state->Le, out, state->buf,
                            state->partial_len);
    }
...
/* ------------------------------------------------------------------------- */
/* Aead interface for Areion512-OPP-MEM                                       */

#include <assert.h>

#include <cryptography-run/aead.h>

static const uint8_t aead_areion512_opp_mem_key_len = 32;
static const uint8_t aead_areion512_opp_mem_nonce_len = 32;
static const uint8_t aead_areion512_opp_mem_overhead = 32;

// Areion512-OPP-MEM does not support key-only initialization, so we just store
// the key and do initialization in encrypt and decrypt which have access to the
// nonce.
typedef struct {
  alignas(32) uint8_t key[32];
} areion512_opp_mem_ctx;

int aead_areion512_opp_mem_init(AeadKey *aead_key,
                                const uint8_t *key,
                                size_t key_len) {
  areion512_opp_mem_ctx *ctx = (areion512_opp_mem_ctx *)aead_key;
  memcpy(ctx->key, key, aead_areion512_opp_mem_key_len);
  return 1;
}

int aead_areion512_opp_mem_seal_scatter(AeadKey *aead_key,
                                        uint8_t *ctcore,
                                        uint8_t *tag,
                                        const uint8_t *msg,
                                        size_t msg_len,
                                        ...
                                        const uint8_t *nonce) {
  areion512_opp_mem_ctx *ctx = (areion512_opp_mem_ctx *)aead_key;
  encrypt_areion_512_opp(ctcore, tag, ad, ad_len, msg, msg_len, nonce,
                         ctx->key);
  return 1;
}

int aead_areion512_opp_mem_partial_open(AeadKey *aead_key,
                                        uint8_t *msg,
                                        uint8_t *tag,
                                        ...
                                        const uint8_t *nonce) {
  areion512_opp_mem_ctx *ctx = (areion512_opp_mem_ctx *)aead_key;
  return partial_decrypt_areion_512_opp(msg, tag, ad, ad_len, ctcore,
                                        ctcore_len, nonce, ctx->key);
}

int aead_areion512_opp_mem_seal(AeadKey *aead_key,
                                uint8_t *ct,
                                const uint8_t *msg,
                                size_t msg_len,
                                const uint8_t *ad,
                                ...
                                const uint8_t *nonce) {
  areion512_opp_mem_ctx *ctx = (areion512_opp_mem_ctx *)aead_key;
  encrypt_areion_512_opp(ct, ct + msg_len, ad, ad_len, msg, msg_len, nonce,
                         ctx->key);
  return 1;
}

int aead_areion512_opp_mem_open(AeadKey *aead_key,
                                uint8_t *msg,
                                const uint8_t *ct,
                                size_t ct_len,
                                const uint8_t *ad,
                                ...
                                const uint8_t *nonce) {
  areion512_opp_mem_ctx *ctx = (areion512_opp_mem_ctx *)aead_key;
  return decrypt_areion_512_opp(msg,
                                ct + ct_len - aead_areion512_opp_mem_overhead,
                                ad, ad_len, ct, ct_len, nonce, ctx->key);
}

static const Aead aead_areion512_opp_mem = {
    "Areion512-OPP-MEM",
    aead_areion512_opp_mem_key_len,
    aead_areion512_opp_mem_nonce_len,
    aead_areion512_opp_mem_overhead,
    aead_areion512_opp_mem_init,
    aead_areion512_opp_mem_seal,
    aead_areion512_opp_mem_open,
    aead_areion512_opp_mem_seal_scatter,
    aead_areion512_opp_mem_partial_open};

const Aead *Aead_areion512_opp_mem() {
  return &aead_areion512_opp_mem;
}
