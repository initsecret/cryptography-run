...
void encrypt_areion_256_opp(uint8_t *c,
...
int decrypt_areion_256_opp(uint8_t *m,
...
bool initialize_areion_256_opp(bool enc,
                               const uint8_t *n,
                               const uint8_t *k,
...
bool update_areion_256_opp(uint8_t *out,
                           size_t *olen,
                           size_t olimit,
                           const uint8_t *in,
                           size_t ilen,
...
bool finalize_areion_256_opp(uint8_t *out,
                             size_t *olen,
                             size_t olimit,
                             uint8_t *tag,
                             areion_256_opp_t *state) {
...
/* ------------------------------------------------------------------------- */
/* Aead interface for Areion256-OPP-MEM                                       */

#include <assert.h>

#include <cryptography-run/aead.h>

static const uint8_t aead_areion256_opp_mem_key_len = 16;
static const uint8_t aead_areion256_opp_mem_nonce_len = 16;
static const uint8_t aead_areion256_opp_mem_overhead = 16;

// Areion256-OPP-MEM does not support key-only initialization, so we just store
// the key and do initialization in encrypt and decrypt which have access to the
// nonce.
typedef struct {
  alignas(32) uint8_t key[32];
} areion256_opp_mem_ctx;

int aead_areion256_opp_mem_init(AeadKey *aead_key,
                                const uint8_t *key,
                                size_t key_len) {
  areion256_opp_mem_ctx *ctx = (areion256_opp_mem_ctx *)aead_key;
  memcpy(ctx->key, key, aead_areion256_opp_mem_key_len);
  return 1;
}

int aead_areion256_opp_mem_seal_scatter(AeadKey *aead_key,
                                        uint8_t *ctcore,
                                        uint8_t *tag,
                                        const uint8_t *msg,
                                        size_t msg_len,
                                        ...
                                        const uint8_t *nonce) {
  areion256_opp_mem_ctx *ctx = (areion256_opp_mem_ctx *)aead_key;
  encrypt_areion_256_opp(ctcore, tag, ad, ad_len, msg, msg_len, nonce,
                         ctx->key);
  return 1;
}

int aead_areion256_opp_mem_partial_open(AeadKey *aead_key,
                                        uint8_t *msg,
                                        uint8_t *tag,
                                        ...
                                        const uint8_t *nonce) {
  areion256_opp_mem_ctx *ctx = (areion256_opp_mem_ctx *)aead_key;
  return decrypt_areion_256_opp(msg, tag, ad, ad_len, ctcore, ctcore_len, nonce,
                                ctx->key);
}

int aead_areion256_opp_mem_seal(AeadKey *aead_key,
                                uint8_t *ct,
                                const uint8_t *msg,
                                size_t msg_len,
                                const uint8_t *ad,
                                ...
                                const uint8_t *nonce) {
  areion256_opp_mem_ctx *ctx = (areion256_opp_mem_ctx *)aead_key;
  encrypt_areion_256_opp(ct, ct + msg_len, ad, ad_len, msg, msg_len, nonce,
                         ctx->key);
  return 1;
}

int aead_areion256_opp_mem_open(AeadKey *aead_key,
                                uint8_t *msg,
                                const uint8_t *ct,
                                size_t ct_len,
                                const uint8_t *ad,
                                ...
                                const uint8_t *nonce) {
  areion256_opp_mem_ctx *ctx = (areion256_opp_mem_ctx *)aead_key;
  return decrypt_areion_256_opp(msg,
                                ct + ct_len - aead_areion256_opp_mem_overhead,
                                ad, ad_len, ct, ct_len, nonce, ctx->key);
}

static const Aead aead_areion256_opp_mem = {
    "Areion256-OPP-MEM",
    aead_areion256_opp_mem_key_len,
    aead_areion256_opp_mem_nonce_len,
    aead_areion256_opp_mem_overhead,
    aead_areion256_opp_mem_init,
    aead_areion256_opp_mem_seal,
    aead_areion256_opp_mem_open,
    aead_areion256_opp_mem_seal_scatter,
    aead_areion256_opp_mem_partial_open};

const Aead *Aead_areion256_opp_mem() {
  return &aead_areion256_opp_mem;
}
