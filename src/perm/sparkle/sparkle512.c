...
static void clear_state_opt(uint32_t *state, int brans) {
  int i;
...
static void print_state_opt(const uint32_t *state, int brans) {
  uint8_t *sbytes = (uint8_t *)state;
  int i, j;

  for (i = 0; i < brans; i++) {
    j = 8 * i;
    printf("(%02x%02x%02x%02x %02x%02x%02x%02x)", sbytes[j], sbytes[j + 1],
           sbytes[j + 2], sbytes[j + 3], sbytes[j + 4], sbytes[j + 5],
           sbytes[j + 6], sbytes[j + 7]);
    if (i < brans - 1)
      printf(" ");
  }
  printf("\n");
...
static void test_sparkle_opt(int brans, int steps) {
  uint32_t state[2 * MAX_BRANCHES] = {0};

  printf("input:\n");
  print_state_opt(state, brans);
  sparkle512_opt(state, brans, steps);
  printf("sparkle:\n");
  print_state_opt(state, brans);
  sparkle512_inv_opt(state, brans, steps);
  printf("sparkle inv:\n");
  print_state_opt(state, brans);
  printf("\n");
}
