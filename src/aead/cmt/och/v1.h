...
#define V1_ALPHA_UPDATE_1(L)                \
  do {                                      \
    L[16] = ROT64(L[0], 11) ^ (L[5] << 13); \
  } while (0)

#define V1_ALPHA_UPDATE_2(L)   \
  do {                         \
    int i;                     \
    for (i = 0; i < 16; ++i) { \
      L[i] = L[i + 1];         \
...
#define V1_GAMMA_UPDATE(L)                                               \
  do {                                                                   \
    L[16] = ROT64(L[0], 11) ^ (L[5] << 13);                              \
    L[17] = ROT64(L[1], 11) ^ (L[6] << 13);                              \
    STOREU256(&L[0], XOR256(XOR256(LOADU256(&L[0]), LOADU256(&L[1])),    \
                            LOADU256(&L[2])));                           \
    STOREU256(&L[4], XOR256(XOR256(LOADU256(&L[4]), LOADU256(&L[5])),    \
                            LOADU256(&L[6])));                           \
    STOREU256(&L[8], XOR256(XOR256(LOADU256(&L[8]), LOADU256(&L[9])),    \
                            LOADU256(&L[10])));                          \
    STOREU256(&L[12], XOR256(XOR256(LOADU256(&L[12]), LOADU256(&L[13])), \
                             LOADU256(&L[14])));                         \
  } while (0)
