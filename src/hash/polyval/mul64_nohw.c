// Adapted from
// https://github.com/google/boringssl/blob/55de8cd4a0405a059bf2c1d56773ec12e5d80b25/crypto/fipsmodule/aes/gcm_nohw.cc.inc
...
// Use native uint128 on supported platforms
// from
// https://github.com/google/boringssl/blob/808b8f3b9d71a055f741d5f7338d206e98747950/crypto/internal.h#L87-L103
#if defined(__x86_64__)  // || defined(__aarch64__)
#define CR_HAS_UINT128
typedef __int128_t int128_t;
typedef __uint128_t uint128_t;
#endif

#if defined(CR_HAS_UINT128)
...
uint64_t bssl_gcm_mul32_nohw(uint32_t a, uint32_t b) {
...
void bssl_gcm_mul64_nohw(uint64_t *out_lo,
  ...
  uint64_t lo = bssl_gcm_mul32_nohw(a0, b0);
  uint64_t hi = bssl_gcm_mul32_nohw(a1, b1);
  uint64_t mid = bssl_gcm_mul32_nohw(a0 ^ a1, b0 ^ b1) ^ lo ^ hi;
  *out_lo = lo ^ (mid << 32);
  *out_hi = hi ^ (mid >> 32);
}

#endif  // CR_HAS_UINT128

// Htable is generated so output
// Xi = the POLYVAL key
static void bssl_gcm_init_nohw(u128_t Htable[16], const uint64_t Xi[2]) {
...
void bssl_gcm_polyval_nohw(uint64_t Xi[2], const u128_t *H) {
  ...
  // evaluating POLYVAL.
  uint64_t r0, r1;
  bssl_gcm_mul64_nohw(&r0, &r1, Xi[0], H->lo);
  uint64_t r2, r3;
  bssl_gcm_mul64_nohw(&r2, &r3, Xi[1], H->hi);
  uint64_t mid0, mid1;
  bssl_gcm_mul64_nohw(&mid0, &mid1, Xi[0] ^ Xi[1], H->hi ^ H->lo);
  mid0 ^= r0 ^ r2;
  mid1 ^= r1 ^ r3;
  r2 ^= mid1;
  r1 ^= mid0;
